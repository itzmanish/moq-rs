...
Relative and Absolute Joining FETCH requests are accepted from downstream clients, resolved from the associated subscription state, and forwarded upstream as fresh Standalone FETCH requests. V1 does not forward Joining FETCH natively between hops or serve FETCH responses from the track cache.
