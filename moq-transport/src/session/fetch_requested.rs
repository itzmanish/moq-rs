...
mod tests {
    ...
    async fn publisher_to_relay_to_subscriber_copies_body() {
        ...
        let (mut upstream, mut upstream_recv) = super::super::Fetch::new(subscriber, request(64));
        let body = bytes::Bytes::from_static(b"opaque fetch body");
        let send_body = {
            let body = body.clone();
            async move {
                let mut writer = Writer::new(
                    SessionId::generate(),
                    upstream_send.open_uni().await.unwrap(),
                );
                writer.write(&body).await.unwrap();
                writer.finish().unwrap();
            }
        };
        let receive_body = async {
            let stream = upstream_receive.accept_uni().await.unwrap();
            upstream_recv
                .recv_stream(super::super::Reader::new(SessionId::generate(), stream))
                .unwrap();
        };
        tokio::join!(send_body, receive_body);
        upstream_recv
        ...
        let (downstream_send, downstream_receive, _downstream_client, _downstream_server) =
            webtransport_pair().await;
        ...
        let receive = async {
            ...
            let header: StreamHeader = reader.decode().await.unwrap();
            assert_eq!(header.fetch_header.unwrap().request_id, 7);
            let mut received = bytes::BytesMut::new();
            while let Some(chunk) = reader.read_chunk(1024).await.unwrap() {
                received.extend_from_slice(&chunk);
            }
            received.freeze()
        };
        let (result, received) = tokio::join!(proxy, receive);

        result.unwrap();
        assert_eq!(received, body);
        let Message::FetchOk(ok) = outgoing.pop().await.unwrap() else {
